<template>
    <AdminLayout :title="`Editar: ${skill.name}`">
        <div class="page">
            <div class="page-header">
                <Link href="/admin/skills" class="back-link">
                    ← Volver a habilidades
                </Link>
                <h2 class="page-title">Editar habilidad</h2>
            </div>

            <form class="form-card" @submit.prevent="submit">
                <!-- Imagen con preview -->
                <div class="field">
                    <label class="field-label">Imagen de la habilidad</label>

                    <div class="image-upload" @click="triggerFileInput">
                        <!-- Muestra preview nuevo o imagen existente -->
                        <img
                            v-if="preview || skill.icon"
                            :src="preview || `/storage/${skill.icon}`"
                            alt="Preview"
                            class="image-preview"
                        />
                ...
                <div class="form-actions">
                    <Link href="/admin/skills" class="btn-ghost"
                        >Cancelar</Link
                    >
                    <button
                        type="submit"
...
async function submit() {
  form.post(`/admin/skills/${props.skill.id}`, {
    forceFormData: true,
    onSuccess: () => {
      Swal.fire({
        title: '¡Skill actualizada!',
        text: 'La skill se actualizó correctamente.',
        icon: 'success',
    ...
    onError: () => {
      Swal.fire({
        title: 'Error',
        text: 'Hubo un problema al actualizar la skill.',
        icon: 'error',
        confirmButtonText: 'Aceptar'
      })
    }
